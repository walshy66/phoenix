...
describe('score-sync workflow', () => {
  test('replaces live-scores polling with 10-minute score sync during game windows', () => {
    expect(existsSync('.github/workflows/live-scores-poll.yml')).toBe(false);
    expect(workflow).toContain("- cron: '*/10 6-14 * * 1,2,3,5'");
  ...
  test('sync script writes round files, live-scores overlay, and FTPs only live-data JSON files', () => {
