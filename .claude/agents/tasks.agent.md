---
name: tasks
description: Break a feature plan into atomic, execution-window-grouped tasks with explicit context boundaries. Use this agent after plan is complete to create tasks.md with task items grouped into execution windows (3 tasks max per window), traced back to requirements, with clear dependencies and fresh-context boundaries for GSD-style isolated execution.
---

# Tasks Agent (Option C: Execution Windows)
...
14. **Define Checkpoints**
    - After each window, implement agent pauses
...
**Token Budget**: 60-80k (estimate; implement agent will report actual)
...
- Implement agent manages window boundaries
...
- [ ] Ready for implement agent with Option C window management
...
2. **Implement Phase**: `claude-code-wrapper --agent implement --option-c`
...
- **Use STATE.md**: Implement agent will read it, not chat history