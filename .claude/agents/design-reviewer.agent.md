...
description: Validate a Gemini-written spec for structure, constitution compliance, and testability. This agent ONLY reviews—it does not rewrite. Use after running design-gemini prompt to catch issues before proceeding.
---
...
```bash
claude-code-wrapper --agent design-reviewer
```
...
**Or direct:**
```bash
claude-code-wrapper "Review spec for feature 045"
```
...
11. **Do NOT Modify Files**
    ...
    - Do not propose changes (that's clarify agent job)
...
3. (If critical) Use clarify agent to fix before planning
...
2. OR use clarify agent if you want to tighten spec first:
   ```bash
...
| **WARN** | Minor issues, can proceed | Flag for later or use clarify agent |
